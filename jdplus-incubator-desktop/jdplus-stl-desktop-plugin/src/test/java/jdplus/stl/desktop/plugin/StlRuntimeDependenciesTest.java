package jdplus.stl.desktop.plugin;
...
public class StlRuntimeDependenciesTest {

    @Test
    public void test() throws IOException {
        assertThat(getRuntimeDependencies(StlRuntimeDependenciesTest.class))
                .describedAs("Check runtime dependencies")
                .satisfies(StlRuntimeDependenciesTest::checkStl)
                .hasSize(3);
    }

    private static void checkStl(List<? extends GAV> coordinates) {
        assertThatGroupId(coordinates, "eu.europa.ec.joinup.sat")
                .has(sameVersion())
                .extracting(GAV::getArtifactId)
                .are(matchingPattern(compile("^jdplus-stl-base-\\w+$")))
                .hasSize(3);
    }

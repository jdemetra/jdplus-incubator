package jdplus.jvsqr.desktop.plugin;
...
public class JvsqrRuntimeDependenciesTest {

    @Test
    public void test() throws IOException {
        assertThat(getRuntimeDependencies(JvsqrRuntimeDependenciesTest.class))
                .describedAs("Check runtime dependencies")
                .satisfies(JvsqrRuntimeDependenciesTest::checkSts)
                .hasSize(2);
    }

    private static void checkSts(List<? extends GAV> coordinates) {
        assertThatGroupId(coordinates, "eu.europa.ec.joinup.sat")
                .has(sameVersion())
                .extracting(GAV::getArtifactId)
                .are(matchingPattern(compile("^jdplus-jvsqr-base-\\w+$")))
                .hasSize(2);
    }

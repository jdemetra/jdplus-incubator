...
public interface HighFreqDiagnosticsFactory<C extends DiagnosticsConfiguration, R> extends DiagnosticsFactory<C, R> {
    ...
    public static class Ordering implements Comparator<HighFreqDiagnosticsFactory> {

        @Override
        public int compare(HighFreqDiagnosticsFactory diag1, HighFreqDiagnosticsFactory diag2) {
    ...
    @Override
    default HighFreqDiagnosticsFactory<C, R> activate(boolean active) {
        if (isActive() == active) {
            return this;
        } else {
    ...
    @Override
    HighFreqDiagnosticsFactory<C, R> with(C newConfig);
